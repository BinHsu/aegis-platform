...
| `ack-iam.tf` + `irsa-ack-iam.tf` | ACK IAM controller (iam-chart 1.3.13, ns `ack-system`) + its IRSA role `aegis-platform-aws-ack-iam-${region}` (scoped to the `/aegis-workload/` IAM path). Lets a deploy repo declare its own IAM as `Role`/`Policy` CRDs. The role name's `aegis-platform-aws-ack-iam-` prefix is the seam to the fabric SCP carve-out. ADR-07. **E2E PENDING bootstrap.** |
